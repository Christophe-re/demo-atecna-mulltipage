...
const defaultComponents = {
  ...
  heading3: ({ children }) => (
    <Heading
      as="h3"
      size="sm"
      className="mb-7 mt-12 first:mt-0 last:mb-0 text-atlantique"
    >
      {children}
    </Heading>
  ),
  paragraph: ({ children }) => (
    <p className="mb-7 last:mb-0 text-atlantique">{children}</p>
  ),
  oList: ({ children }) => (
  ...
  preformatted: ({ children }) => (
    ...
    // </pre>
    <div
      className="mb-1 justify-center sm:hidden md:flex"
